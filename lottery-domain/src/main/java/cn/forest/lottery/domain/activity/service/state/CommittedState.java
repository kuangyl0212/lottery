 ...
 */
@Component
public class CommittedState extends AbstractState {
    @Override
    ...
    public Result approve(Long activityId, Enum<LotteryConstants.ActivityState> currentState) {
        ...
        return isSuccess ? new Result(0, "审核通过") : new Result(1, "通过失败");
    }

    @Override
    public Result deny(Long activityId, Enum<LotteryConstants.ActivityState> currentState) {
        boolean isSuccess = activityRepository.updateState(activityId, currentState, LotteryConstants.ActivityState.DENIED);
        return isSuccess ? new Result(0, "拒绝成功") : new Result(1, "拒绝失败");
    }
